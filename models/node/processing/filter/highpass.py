...
class HighPass(Filter):
    """ This is a highpass filter. It's a node that filters the input data with a highpass filter. A highpass filter
    ...
    Attributes:
        _MODULE_NAME (`str`): The name of the module (in his case ``node.processing.filter.highpass``)
    
    configuration.json usage:
        **module** (*str*): The name of the module (``node.processing.filter``)\n
        **type** (*str*): The type of the node (``highPass``)\n
        **low_cut_frequency_hz** (*float*): The high cut frequency in Hz.\n
    ...
    """
    _MODULE_NAME: Final[str] = 'node.processing.filter.highpass'
